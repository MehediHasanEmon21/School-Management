        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              <h2 class="card-title">Edit Fee Category</h2>
            ...
            <!-- /.card-header -->
            <div class="card-body">
            <form action="{{ route('student.fee.amount.update',$editDatas[0]->fee_category_id) }}" method="POST" id="myform">
                @csrf
              <div class="add_item">
                    ...
                    <div class="form-group col-md-5">
                        ...
                        <select name="fee_category_id" class="form-control">
                          ...
                          @foreach($fee_categories as $fee_category)
                            <option value="{{ $fee_category->id }}" {{ ($fee_category->id == $editDatas[0]->fee_category_id) ? 'selected' : '' }}>{{ $fee_category->name }}</option>
                          @endforeach
                        </select>
                    </div>
                  ...
                  <div class="delete_whole_extra_item_add" id="delete_whole_extra_item_add">
                      <div class="form-row">
                          ...
                          <label>Class</label>
                            <select name="student_class_id[]" class="form-control">
                              ...
                              @foreach($student_classes as $cls)
                                <option value="{{ $cls->id }}" {{ ($cls->id == $editData->student_class_id) ? 'selected' : '' }}>{{ $cls->name }}</option>
                              @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-5">
                          <label>Amount</label>
                          <input type="number" class="form-control" value="{{ $editData->amount }}" name="amount[]" placeholder="Enter amount">
                        </div>
